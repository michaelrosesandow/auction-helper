...
- **Build:** `npm run build` bundles `src/{content,background,popup}.ts` to
  `dist/` via esbuild (format `iife`), copies `manifest.json` + `popup.html`.
  **Load unpacked from `dist/`.** `npm run watch` for rebuild-on-save.
- **Browser globals:** content/background/popup use the `chrome.*` APIs
  (`@types/chrome`) and DOM (`lib: DOM`). oxlint runs with `env: browser+node`
  (build scripts are Node).
- **Rankings are runtime data, not build data.** Import via the popup
